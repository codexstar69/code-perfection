...
Enforced code refactoring and optimization skill. Uses mechanical scripts to drive a resolution loop that cannot exit until all issues are resolved or explicitly deferred.
...
**Trigger phrases:** "refactor", "optimize", "clean up", "code quality", "audit", "code perfection", "resolution loop", "fix all issues"

## Commands

| Command | Description |
|---------|-------------|
| `/code-perfection` | Run the resolution loop on a target (default: current directory) |
| `/code-perfection:audit` | Full tiered codebase audit (100+ files) |
...
- **Simple fix (1–3 files):** Load `references/agents.md` only
- **Multi-issue refactoring:** Load `references/agents.md` + `references/resolution-loop.md`
- **Codebase audit (100+ files):** Load `references/agents.md` + `references/audit.md` + `references/resolution-loop.md`
- **Choosing execution mode:** Load `references/parallel-vs-sequential.md`
...
7. Enter the resolution loop (see `references/resolution-loop.md` for the full protocol):
   ...
   - `"$SKILL_DIR/scripts/resolution-loop.sh" status` — exit code 1 = continue, 0 = done
...
- **`resolution-loop.sh`** — lockfile prevents concurrent writers, auto-reverts on failure, auto-defers after 3 attempts, auto-commits on resolve, checkpoint every 5 fixes. Exit code 1 from `status` = loop must continue.
...
| Max iterations exceeded | Loop exits with summary report |
